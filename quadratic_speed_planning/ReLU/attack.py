...
def ZeroColumnLoss(A):
    idx = torch.zeros(40).long()
    j = torch.arange(40).long()
    update_values = torch.zeros(40)
    A_prime = A.clone()
...
if __name__=="__main__":
    ...
    parser.add_argument("-e", "--epochs", help="Epochs to run the experiment", type=int,  default=500)
    ...
    parser.add_argument("-r", "--seed", help="Random seed to use", type=int,  default=250)
    parser.add_argument("-l", "--logdir",help="Directory to save logs to, together with data", default="./cond_atk_loss_fn")
    parser.add_argument("-f", "--loss-function",help="Loss function to use", default="ConditionNumberLoss")
