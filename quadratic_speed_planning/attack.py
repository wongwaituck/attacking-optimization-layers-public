...
class QPSpeedPlanning(nn.Module):
    ...
    def forward(self, x):
        ...
        G_p = G
        # defence
        if self.epsilon != 0 and self.epsilon != 0.0:
...
def ZeroColumnLoss(A):
    idx = torch.zeros(A.shape[1]).long()
    j = torch.arange(A.shape[1]).long()
    update_values = torch.zeros(A.shape[1])
    A_prime = A.clone()
...
def train(model, device, trainset, Y, optimizer, epoch, batch_sz, logger, lossfn):
    ...
    G_p = G
    # defence
    if model.epsilon != 0 and model.epsilon != 0.0:
        # perturb the matrix a little bit to handle the all zeroes case        
        try:
            U, S, V = torch.svd(A)
            S_p = torch.zeros_like(S)
...
if __name__=="__main__":
    ...
    parser.add_argument("-e", "--epochs", help="Epochs to run the experiment", type=int,  default=5000)
    parser.add_argument("-d", "--datadir", help="Directory to load the data from",  default="out")
    parser.add_argument("-m", "--modeldir", help="Directory to load models from",  default="trial_zero_row")
    parser.add_argument("-r", "--seed", help="Random seed to use", type=int,  default=1)
    parser.add_argument("-l", "--logdir",help="Directory to save logs to, together with data", default="./cond_atk_loss_fn_trial")
    parser.add_argument("-f", "--loss-function",help="Loss function to use", default="ZeroRowLoss")
    parser.add_argument("-b", "--attack-sz", help="Attack size to use", type=int,  default=30)
    ...
    X_attack = X_test[-args.attack_sz:]
    Y_attack = Y_test[-args.attack_sz:]
    ...
    for i in range(args.attack_sz):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            out = train(model, device, torch.clamp(X + delta, 0, 1), Y, optimizer, epoch, 1, LOGS, lossfn)
            if isnan(out.sum().item()):
                ...
                actual_delta = torch.clamp(X + old_delta, 0, 1) - X
                with torch.no_grad():
                    actual_delta[:,56*56*3:] = 0 
                # save the delta
                torch.save(actual_delta, f"{LOGDIR}/adversarial{i}_{args.loss_function}_{args.epsilon}.pt")

                # save the image
                adv_x = torch.clamp(X + old_delta, 0, 1)
                image = adv_x.clone()[:, :IMG_DIMENSIONS[0] * IMG_DIMENSIONS[1] * IMG_DIMENSIONS[2]]
                image = image.reshape(
