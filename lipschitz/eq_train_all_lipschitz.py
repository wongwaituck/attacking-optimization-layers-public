...
class OptNetDef(nn.Module):
    ...
    def forward(self, x):
        ...
        lipschitz_regularizer = torch.clamp(A.norm(dim=[1,2], keepdim=True), 1, math.inf)
        A_p = A / lipschitz_regularizer

        x = QPFunction(verbose=-1)(Q, q, G, h, A_p, b)

        return F.log_softmax(x),torch.linalg.cond(A_p)
...
if (epsilon == 0):
    ...
    outputDir = "model_def_lipschitz/"+ "/seed_" + str(args.seed) + "/"
...
modelCondPath = f"{outputDir}network{nFeatures}f{nHidden}h{nCls}c{args.neq}eqs_cond.csv"
if(os.path.exists(modelPath)):
    ...
    np.savetxt(modelCondPath, np.array(conds))
